...
pub fn summarize_eval(tasks: &[TaskEval], pricing: &PricingConfig, seed: u64) -> EvalSummary {
    ...
    let cost_per_pass = |dollars: &[f64], rate: f64| {
        if rate > 0.0 {
            mean(dollars) / rate
        } else {
            f64::INFINITY
    ...
    let fids: Vec<f64> = tasks
        .iter()
        .filter_map(|t| t.fidelity.map(|f| f as f64))
        .collect();

    let (cost_per_pass_ratio, cost_per_pass_ci) =
        cost_per_pass_ratio_ci(tasks, pricing, 10_000, seed ^ 3);
    ...
    EvalSummary {
        ...
        fidelity_mean: if fids.is_empty() {
            None
        } else {
            Some(mean(&fids))
        },
...
pub fn build_eval_markdown(
    ...
    let _ = writeln!(
        o,
        "- control_mode: `full`  ·  tokenizer: proxy (o200k_base)\n"
    );

    let _ = writeln!(o, "## Headline — three layers (ZH vs EN, paired)\n");
    let _ = writeln!(
        o,
        "| Layer | EN | ZH | Δ% | 95% CI | ZH win-rate | Verdict |"
    );
    let _ = writeln!(o, "|---|---:|---:|---:|---:|---:|---|");
    let row = |o: &mut String, name: &str, l: &LayerStat| {
        let _ = writeln!(
            ...
            l.ci_hi,
            l.win_rate * 100.0,
            verdict(l.ci_lo, l.ci_hi)
        );
    };
    row(&mut o, "input tokens (proxy)", &s.proxy_input);
    row(
        &mut o,
        "input tokens (reported, uncached)",
        &s.reported_input,
    );
    ...
    let _ = writeln!(
        o,
        "| Arm | pass | N | pass-rate | Wilson 95% CI | $ / passing task |"
    );
    let _ = writeln!(o, "|---|---:|---:|---:|---:|---:|");
    let cpp = |v: f64| {
        if v.is_finite() {
    ...
    let _ = writeln!(
        ...
        s.en_pass.passes,
        s.en_pass.n,
        s.en_pass.rate * 100.0,
        s.en_pass.ci_lo * 100.0,
        s.en_pass.ci_hi * 100.0,
        cpp(s.en_cost_per_pass)
    );
    let _ = writeln!(
        o,
        "| ZH | {} | {} | {:.1}% | [{:.1}, {:.1}]% | {} |",
        s.zh_pass.passes,
        s.zh_pass.n,
        s.zh_pass.rate * 100.0,
        s.zh_pass.ci_lo * 100.0,
        s.zh_pass.ci_hi * 100.0,
        cpp(s.zh_cost_per_pass)
    );
    let ratio_str = if s.cost_per_pass_ratio.is_finite() {
        format!("{:.2}×", s.cost_per_pass_ratio)
    } else {
        "∞".into()
    };
    ...
    let _ = writeln!(
        o,
        "- Proxy token counts are `o200k_base`, NOT Claude's tokenizer; treat as directional."
    );
    ...
    let _ = writeln!(
        o,
        "- N = {}; deltas carry bootstrap CIs but remain point estimates of a small sample.",
        s.n
    );
    let _ = writeln!(
        o,
        "- Execution runs model-generated code locally; run untrusted corpora in a VM/container."
    );
...
pub fn build_eval_csv(tasks: &[TaskEval], pricing: &PricingConfig) -> String {
    ...
    for t in tasks {
        for (arm, e) in [("en", &t.en), ("zh", &t.zh)] {
            let fid = if arm == "zh" {
                t.fidelity.map(|v| v.to_string()).unwrap_or_default()
            } else {
                String::new()
            };
            let _ = writeln!(
                ...
                e.cost.cache_read,
                e.cost.cache_write,
                e.cost.marginal(pricing),
                e.cost.billed(pricing),
                fid
            );
        }
    }
...
mod tests {
    ...
    fn sample() -> Vec<TaskEval> {
        vec![
            TaskEval {
                ...
                zh: arm(Outcome::AssertFail, 130, 240, 70),
                fidelity: Some(8),
            },
            TaskEval {
                task_id: "t2".into(),
                category: "coding-verifiable".into(),
                en: arm(Outcome::Pass, 120, 220, 110),
                zh: arm(Outcome::Pass, 150, 250, 85),
                fidelity: Some(6),
            },
        ]
    }
    ...
    fn markdown_and_csv_render() {
        ...
        assert!(csv
            ...
            .next()
            .unwrap()
            .starts_with("task_id,category,arm,outcome"));
    }
}
